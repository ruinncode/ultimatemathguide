(function () {
  ...
  const STRATEGIES = {
    'A.1': { strat: 'Follow BEDMAS/PEMDAS strictly: Brackets first, then Exponents, then Division and Multiplication (left to right), then Addition and Subtraction (left to right).', takeaway: 'Order of operations is the grammar of math. Get it wrong and the whole calculation falls apart. When in doubt, add brackets to clarify.' },
    'A.2': { strat: 'To convert between fractions, decimals, and percents: divide for fractions→decimals, multiply by 100 for decimals→percents. For fraction operations, find the LCD first.', takeaway: 'Fractions, decimals, and percents are three ways to write the same number. Being fluent in all three is essential for everything that follows.' },
    'A.3': { strat: 'Set up a proportion (two equal ratios) and cross-multiply to solve. Always check: does the answer make sense in context?', takeaway: 'Proportional reasoning is one of the most useful skills in everyday life — from cooking to currency conversion to scaling.' },
    'A.4': { strat: 'For exponents, remember the rules: same base multiplied → add exponents; same base divided → subtract; power of a power → multiply. For radicals, find the largest perfect square factor.', takeaway: 'Exponents are repeated multiplication. The rules all follow from this one idea. Simplifying radicals = factoring out perfect squares.' },
    'A.5': { strat: 'For scientific notation, move the decimal until you have a number between 1 and 10, then count the moves as the exponent. For Pythagorean theorem, identify which side is the hypotenuse (longest, opposite the right angle).', takeaway: 'The Pythagorean theorem connects geometry to algebra. It is the foundation of distance formulas, trigonometry, and much of physics.' },
    'B.1': { strat: 'Translate words into symbols step by step. "More than" means +, "less than" means −, "times" means ×, "of" means ×, "is" means =.', takeaway: 'Algebra is a language. Once you can translate between English and symbols, you can solve problems that words alone cannot.' },
    'B.2': { strat: 'To solve an equation: (1) distribute, (2) combine like terms, (3) move variables to one side, (4) move constants to the other, (5) divide. Check by substituting back.', takeaway: 'Solving equations is the core skill of algebra. Whatever you do to one side, you must do to the other. Always check your answer.' },
    'B.3': { strat: 'Solve exactly like an equation, BUT: if you multiply or divide by a NEGATIVE number, you must FLIP the inequality sign. Graph the solution on a number line.', takeaway: 'The only difference between equations and inequalities is the flip rule. Forgetting to flip when dividing by a negative is the most common mistake.' },
    'B.4': { strat: 'To factor: (1) always pull out the GCF first, (2) check for difference of squares (a²−b²), (3) for trinomials, find two numbers that multiply to the constant and add to the middle coefficient.', takeaway: 'Factoring is "un-multiplying." It is the reverse of expanding and is essential for solving quadratic equations and simplifying expressions.' },
    'C.1': { strat: 'Slope = rise/run = (y₂−y₁)/(x₂−x₁). Positive slope goes up-right, negative goes down-right, zero is horizontal, undefined is vertical.', takeaway: 'Slope measures steepness. It is the same concept as the derivative in calculus — you are learning derivatives right now without knowing it!' },
    'C.2': { strat: 'Use slope-intercept form (y = mx+b) when you know slope and y-intercept. Use point-slope form (y−y₁ = m(x−x₁)) when you know a point and slope. Convert between forms as needed.', takeaway: 'The three forms of a linear equation are different views of the same line. Choose the form that matches what you are given.' },
    'C.3': { strat: 'Substitution: solve one equation for a variable, plug into the other. Elimination: add/subtract equations to cancel a variable. Choose whichever looks easier for the given system.', takeaway: 'A system of equations has a solution where the lines intersect. No solution = parallel lines. Infinite solutions = same line.' },
    'D.1': { strat: 'Identify which form the quadratic is in (standard, vertex, or factored), then extract the key features: vertex, axis of symmetry, direction of opening, x-intercepts.', takeaway: 'Quadratics model parabolas — they appear in projectile motion, optimization, and revenue/cost problems. The vertex is always the most important point.' },
    'D.2': { strat: 'Factor by finding two numbers that multiply to ac and add to b. For simple trinomials (a=1), just find two numbers multiplying to c and adding to b.', takeaway: 'Factoring quadratics is a skill you will use hundreds of times. Practice until you can spot the factors quickly.' },
    'D.3': { strat: 'Three methods to solve quadratics: (1) factoring (fastest when possible), (2) quadratic formula (always works), (3) completing the square (useful for vertex form). Check the discriminant first.', takeaway: 'The quadratic formula is your universal backup — it works for ANY quadratic. The discriminant tells you how many solutions to expect before you solve.' },
    'D.4': { strat: 'In vertex form y = a(x−h)² + k: a controls stretch and direction, h shifts horizontally (opposite sign!), k shifts vertically. Start from the parent y = x² and apply transformations.', takeaway: 'All parabolas are just y = x² that has been shifted, stretched, or flipped. Understanding transformations lets you graph any quadratic instantly.' },
    'E.1': { strat: 'For parallel lines cut by a transversal, identify the angle relationship (alternate interior, corresponding, co-interior) and use the appropriate rule (equal or supplementary).', takeaway: 'Angle relationships are the building blocks of geometry proofs. Master the parallel lines rules and triangle angle sum.' },
    'E.2': { strat: 'For congruence, check SSS, SAS, ASA, or AAS. For similarity, check AA (two angles equal). Similar triangles have proportional sides.', takeaway: 'Similarity is incredibly powerful — if two triangles are similar, ALL their corresponding sides are proportional. This connects to trigonometry.' },
    'E.3': { strat: 'SOH CAH TOA: identify which sides you know and which you need, pick the right ratio, then solve. For finding angles, use inverse trig (sin⁻¹, cos⁻¹, tan⁻¹).', takeaway: 'SOH CAH TOA works for any right triangle. These three ratios are the foundation of all trigonometry.' },
    'E.4': { strat: 'Sine law for AAS or ASA (when you have an angle and its opposite side). Cosine law for SAS or SSS (when sine law does not apply).', takeaway: 'Together, sine law and cosine law can solve ANY triangle — not just right triangles. They extend SOH CAH TOA to the general case.' },
    'E.5': { strat: 'Identify the shape, recall its formula, substitute the measurements, and compute. For composite shapes, break them into simpler parts.', takeaway: 'Area and volume formulas are tools — memorize the key ones (circle, triangle, cylinder, sphere) and you can handle any shape by combining them.' },
    'E.6': { strat: 'Distance formula is the Pythagorean theorem in disguise: d = √((Δx)² + (Δy)²). Midpoint is just the average of the coordinates.', takeaway: 'Coordinate geometry connects algebra and geometry — you can prove geometric facts using equations, and solve algebraic problems using graphs.' },
