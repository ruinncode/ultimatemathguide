(function () {
  ...
  const Q = {
    'arithmetic': [
      { q: 'Evaluate: $-3 \\times (-4) + 2$', o: ['$14$', '$-10$', '$10$', '$-14$'], a: 0,
        why: ['$-3 \\times (-4) = 12$, then $12 + 2 = 14$. Negative times negative is positive.', 'You may have done $-3 \\times (-4) = -12$ — but two negatives multiply to a positive.', 'Check: $-3 \\times (-4) = 12$, not $8$. Then $12 + 2 = 14$.', 'You multiplied correctly but may have subtracted instead of adding.'] },
      { q: 'Convert $\\frac{3}{8}$ to a decimal:', o: ['$0.375$', '$0.38$', '$0.275$', '$0.35$'], a: 0,
        why: ['$3 \\div 8 = 0.375$. Correct!', '$0.38$ is a rounded approximation, not exact.', '$0.275$ would be $\\frac{11}{40}$, not $\\frac{3}{8}$.', '$0.35 = \\frac{7}{20}$, not $\\frac{3}{8}$.'] },
      { q: 'Simplify: $\\sqrt{50}$', o: ['$5\\sqrt{2}$', '$25$', '$2\\sqrt{5}$', '$10\\sqrt{5}$'], a: 0,
        why: ['$\\sqrt{50} = \\sqrt{25 \\cdot 2} = 5\\sqrt{2}$. Factor out the largest perfect square.', '$25 = \\sqrt{625}$, not $\\sqrt{50}$.', 'Check: $(2\\sqrt{5})^2 = 4 \\cdot 5 = 20 \\neq 50$.', '$(10\\sqrt{5})^2 = 500 \\neq 50$. Way too large.'] },
      { q: 'A triangle has legs 6 and 8. The hypotenuse is:', o: ['$10$', '$14$', '$\\sqrt{48}$', '$100$'], a: 0,
        why: ['$c = \\sqrt{6^2 + 8^2} = \\sqrt{36 + 64} = \\sqrt{100} = 10$. This is a 3-4-5 triple scaled by 2.', 'You added $6 + 8 = 14$, but Pythagorean theorem uses squares, not addition.', '$\\sqrt{48} = \\sqrt{6^2 + (\\sqrt{12})^2}$. You need $6^2 + 8^2$, not $6^2 + 12$.', '$100 = 6^2 + 8^2$ but you forgot to take the square root!'] },
      { q: 'Write $0.00042$ in scientific notation:', o: ['$4.2 \\times 10^{-4}$', '$42 \\times 10^{-5}$', '$4.2 \\times 10^{4}$', '$0.42 \\times 10^{-3}$'], a: 0,
        why: ['Move decimal 4 places right: $4.2 \\times 10^{-4}$. The coefficient must be between 1 and 10.', 'The coefficient must be between 1 and 10. $42$ is too big.', 'Positive exponent means a large number. This is a small number, so exponent must be negative.', '$0.42$ is less than 1, so this is not proper scientific notation.'] },
      { q: 'Evaluate: $\\frac{2}{3} + \\frac{3}{4}$', o: ['$\\frac{17}{12}$', '$\\frac{5}{7}$', '$\\frac{5}{12}$', '$\\frac{6}{12}$'], a: 0,
        why: ['LCD is 12: $\\frac{8}{12} + \\frac{9}{12} = \\frac{17}{12}$. Correct!', 'You added numerators and denominators separately — never do that with fractions.', '$\\frac{5}{12}$ would be $\\frac{2}{12} + \\frac{3}{12}$, but you need a common denominator first.', '$\\frac{6}{12} = \\frac{1}{2}$. Not the right computation.'] },
      { q: 'What is $(-2)^3$?', o: ['$-8$', '$8$', '$-6$', '$6$'], a: 0,
        why: ['$(-2)^3 = (-2)(-2)(-2) = -8$. Odd power of a negative is negative.', '$8 = 2^3$ but the base is $-2$. Odd exponent keeps the negative sign.', 'That is $(-2) \\times 3 = -6$. Exponents mean repeated multiplication, not $\\times 3$.', 'Same error: this is multiplication not exponentiation.'] },
      { q: 'Which proportion is set up correctly to find $x$ if $\\frac{3}{5} = \\frac{x}{20}$?', o: ['$x = 12$', '$x = 15$', '$x = 4$', '$x = 60$'], a: 0,
        why: ['Cross multiply: $3 \\times 20 = 5x$, so $60 = 5x$, giving $x = 12$.', 'You divided $20 \\div 4 = 5$, then multiplied $5 \\times 3 = 15$. The scale factor is $4$, not $5$.', '$20 \\div 5 = 4$ but that is the scale factor, not $x$. Multiply: $3 \\times 4 = 12$.', '$3 \\times 20 = 60$ but you forgot to divide by 5.'] },
    ],
    'intro-algebra': [
      { q: 'Solve: $3x + 7 = 22$', o: ['$x = 5$', '$x = 15$', '$x = 29/3$', '$x = -5$'], a: 0,
        why: ['$3x = 22 - 7 = 15$, so $x = 5$. Subtract 7 from both sides, then divide by 3.', 'You subtracted 7 but forgot to divide by 3.', 'You added 7 instead of subtracting. $22 + 7 = 29$, then $29/3$.', 'Sign error: $22 - 7 = 15$, not $-15$.'] },
      { q: 'Expand: $(x + 3)(x - 5)$', o: ['$x^2 - 2x - 15$', '$x^2 - 15$', '$x^2 + 2x - 15$', '$x^2 - 8x + 15$'], a: 0,
        why: ['FOIL: $x^2 - 5x + 3x - 15 = x^2 - 2x - 15$. Correct!', 'You forgot the middle terms ($-5x + 3x$). Always FOIL completely.', 'The middle term is $-5x + 3x = -2x$, not $+2x$. Watch the signs.', 'This would be $(x - 3)(x - 5)$. The $+3$ makes the middle term different.'] },
      { q: 'Factor: $x^2 - 9$', o: ['$(x+3)(x-3)$', '$(x-3)^2$', '$(x+9)(x-1)$', '$(x-9)(x+1)$'], a: 0,
        why: ['Difference of squares: $a^2 - b^2 = (a+b)(a-b)$. Here $a=x, b=3$.', '$(x-3)^2 = x^2 - 6x + 9 \\neq x^2 - 9$. No middle term means it is NOT a perfect square.', 'Check: $(x+9)(x-1) = x^2 + 8x - 9 \\neq x^2 - 9$.', 'Check: $(x-9)(x+1) = x^2 - 8x - 9 \\neq x^2 - 9$.'] },
      { q: 'Solve: $-2x + 5 > 11$', o: ['$x < -3$', '$x > -3$', '$x < 3$', '$x > 3$'], a: 0,
        why: ['$-2x > 6$, divide by $-2$ and FLIP the inequality: $x < -3$. Correct!', 'You forgot to flip the inequality when dividing by a negative number.', 'Check: $-2(3) + 5 = -1$, which is NOT $> 11$. Wrong value.', '$-2x > 6$ means $x < -3$, not $x > 3$. Two errors: sign and flip.'] },
      { q: 'Factor: $3x^2 - 12$', o: ['$3(x+2)(x-2)$', '$(3x+6)(x-2)$', '$3(x-2)^2$', '$(x+2)(3x-6)$'], a: 0,
        why: ['GCF first: $3(x^2 - 4) = 3(x+2)(x-2)$. Always factor out the GCF before anything else.', 'This expands to $3x^2 - 12x + 12 \\neq 3x^2 - 12$. Not correct.', '$3(x-2)^2 = 3(x^2 - 4x + 4) = 3x^2 - 12x + 12 \\neq 3x^2 - 12$.', 'This equals $3x^2 - 6x + 2x - 12 = 3x^2 - 4x - 12 \\neq 3x^2 - 12$.'] },
      { q: 'Solve: $5(x - 2) = 3x + 6$', o: ['$x = 8$', '$x = 2$', '$x = -8$', '$x = 4$'], a: 0,
        why: ['$5x - 10 = 3x + 6$, so $2x = 16$, giving $x = 8$. Distribute, collect terms, divide.', 'If $x = 2$: $5(0) = 6 + 6 = 12$. $0 \\neq 12$.', 'Sign error: $-10 - 6 = -16$, not $+16$, then $x = -16/2 = -8$... but you also got the subtraction wrong.', '$5(4-2) = 10$ and $3(4)+6 = 18$. $10 \\neq 18$.'] },
      { q: 'What is the degree of $4x^3 - 2x^5 + x - 7$?', o: ['$5$', '$3$', '$4$', '$7$'], a: 0,
        why: ['The degree is the highest exponent. $-2x^5$ has exponent 5 — that is the degree.', 'The $4x^3$ term has exponent 3, but $-2x^5$ has exponent 5 which is higher.', 'There is no $x^4$ term in this polynomial. Degree = highest power present.', '7 is the constant term, not the degree.'] },
      { q: 'Factor completely: $x^2 + 10x + 25$', o: ['$(x+5)^2$', '$(x+25)(x+1)$', '$(x+10)(x+2.5)$', '$(x-5)^2$'], a: 0,
        why: ['Perfect square trinomial: $x^2 + 2(5)x + 5^2 = (x+5)^2$. Check: is the middle term $2 \\times \\sqrt{25} = 10$? Yes!', '$25 \\times 1 = 25$ but $25 + 1 = 26 \\neq 10$.', 'Not integer factors — and $10 \\times 2.5 = 25$ but that is not how trinomial factoring works.', '$(x-5)^2 = x^2 - 10x + 25$. Wrong sign on the middle term.'] },
    ],
    'linear-relations-ch': [
      { q: 'The slope between $(2, 3)$ and $(5, 9)$ is:', o: ['$2$', '$3$', '$6$', '$1/2$'], a: 0,
        why: ['Slope $= \\frac{9-3}{5-2} = \\frac{6}{3} = 2$. Rise over run.', 'You computed the run ($5-2=3$) but that is the denominator, not the slope.', '$6$ is the rise ($9-3$) but you forgot to divide by the run.', 'You flipped the fraction: it is $\\frac{6}{3}$, not $\\frac{3}{6}$.'] },
      { q: 'A line has slope $3$ and passes through $(1, 5)$. Its equation is:', o: ['$y = 3x + 2$', '$y = 3x + 5$', '$y = 3x - 2$', '$y = 5x + 3$'], a: 0,
        why: ['$y - 5 = 3(x - 1) \\Rightarrow y = 3x - 3 + 5 = 3x + 2$. Correct!', 'You used $b = 5$ but the y-intercept is not 5 — the point $(1,5)$ is not on the y-axis.', '$y = 3x - 2$ would pass through $(1, 1)$, not $(1, 5)$.', 'You swapped the slope and the point coordinates.'] },
      { q: 'Solve by substitution: $y = 2x + 1$ and $3x + y = 16$', o: ['$(3, 7)$', '$(5, 11)$', '$(3, 5)$', '$(7, 3)$'], a: 0,
        why: ['Substitute: $3x + (2x+1) = 16 \\Rightarrow 5x = 15 \\Rightarrow x = 3$, then $y = 7$. Correct!', '$x = 5$ does not satisfy $3(5) + (2(5)+1) = 26 \\neq 16$.', '$y$ should be $2(3)+1 = 7$, not $5$.', 'You swapped $x$ and $y$ in the answer.'] },
      { q: 'Two lines with the same slope but different y-intercepts are:', o: ['Parallel', 'Perpendicular', 'Identical', 'Intersecting'], a: 0,
        why: ['Same slope = same direction = parallel. Different y-intercepts means they never meet.', 'Perpendicular lines have slopes that are negative reciprocals ($m_1 \\cdot m_2 = -1$).', 'Identical lines have the same slope AND the same y-intercept.', 'Parallel lines never intersect (unless they are the same line).'] },
      { q: 'Convert $3x + 2y = 12$ to slope-intercept form:', o: ['$y = -\\frac{3}{2}x + 6$', '$y = \\frac{3}{2}x + 6$', '$y = -3x + 12$', '$y = -\\frac{3}{2}x + 12$'], a: 0,
        why: ['$2y = -3x + 12$, so $y = -\\frac{3}{2}x + 6$. Subtract $3x$, then divide everything by 2.', 'The slope should be negative: when you subtract $3x$ from both sides, it becomes $-3x$.', 'You forgot to divide by 2: $2y = -3x + 12$ means $y = -\\frac{3}{2}x + 6$.', 'The y-intercept is $12/2 = 6$, not 12.'] },
      { q: 'The slope of a line perpendicular to $y = 2x + 5$ is:', o: ['$-1/2$', '$2$', '$1/2$', '$-2$'], a: 0,
        why: ['Perpendicular slopes are negative reciprocals: $m = 2$, so $m_{\\perp} = -\\frac{1}{2}$. Flip and negate.', 'That is the same slope — this would be parallel, not perpendicular.', 'You flipped but forgot to negate. Must be $-1/2$, not $+1/2$.', 'You negated but forgot to flip. Must be $-1/2$, not $-2$.'] },
      { q: 'A system has no solution when the lines are:', o: ['Parallel', 'Perpendicular', 'Intersecting at one point', 'The same line'], a: 0,
        why: ['Parallel lines never meet, so there is no intersection point = no solution.', 'Perpendicular lines always intersect (at a 90 degree angle) = one solution.', 'Intersecting lines meet at exactly one point = one solution.', 'The same line means every point is a solution = infinitely many solutions.'] },
    ],
    'quadratics-ch': [
      { q: 'The vertex of $y = 2(x-3)^2 + 4$ is:', o: ['$(3, 4)$', '$(-3, 4)$', '$(3, -4)$', '$(2, 3)$'], a: 0,
        why: ['In vertex form $y = a(x-h)^2 + k$, the vertex is $(h, k) = (3, 4)$.', 'The $-3$ in $(x-3)$ means $h = +3$, not $-3$. Common sign confusion.', '$k = +4$, not $-4$. The $+4$ shifts up.', '$(2, 3)$ comes from reading the wrong numbers.'] },
      { q: 'Factor: $x^2 + 5x + 6$', o: ['$(x+2)(x+3)$', '$(x+1)(x+6)$', '$(x-2)(x-3)$', '$(x+5)(x+1)$'], a: 0,
        why: ['Find two numbers that multiply to 6 and add to 5: $2 \\times 3 = 6$, $2 + 3 = 5$. Correct!', '$1 \\times 6 = 6$ but $1 + 6 = 7 \\neq 5$.', '$(-2)(-3) = 6$ but $-2 + (-3) = -5 \\neq +5$. Wrong signs.', '$5 \\times 1 = 5 \\neq 6$. The product must be 6.'] },
      { q: 'The discriminant of $x^2 - 4x + 4 = 0$ is:', o: ['$0$', '$32$', '$-32$', '$16$'], a: 0,
        why: ['$\\Delta = (-4)^2 - 4(1)(4) = 16 - 16 = 0$. One repeated real root.', 'You may have computed $(-4)^2 + 4(1)(4) = 32$. It is MINUS $4ac$.', '$\\Delta$ is non-negative here. $16 - 16 = 0 \\geq 0$.', '$16 = b^2$ only. You forgot to subtract $4ac$.'] },
      { q: 'Solve: $x^2 - 5x + 6 = 0$', o: ['$x = 2$ or $x = 3$', '$x = -2$ or $x = -3$', '$x = 1$ or $x = 6$', '$x = 5$ or $x = 6$'], a: 0,
        why: ['Factor: $(x-2)(x-3) = 0$, so $x = 2$ or $x = 3$. Correct!', 'These give $x^2 + 5x + 6 = 0$ (positive middle term), not $-5x$.', '$1 \\times 6 = 6$ but $1 + 6 = 7 \\neq 5$.', 'Neither 5 nor 6 satisfies the equation. Check: $25 - 25 + 6 = 6 \\neq 0$.'] },
      { q: 'Complete the square: $x^2 + 6x + 2 = 0$ becomes:', o: ['$(x+3)^2 = 7$', '$(x+3)^2 = 11$', '$(x+6)^2 = 34$', '$(x+3)^2 = -7$'], a: 0,
        why: ['$x^2 + 6x + 9 - 9 + 2 = 0 \\Rightarrow (x+3)^2 = 7$. Half of 6 is 3, square it to get 9.', 'You added 9 but forgot to subtract it: $9 + 2 = 11$, but we moved 9 to the right giving $9 - 2 = 7$.', 'Half of 6 is 3, not 6. Square HALF the coefficient.', 'The right side is positive ($7$), not negative.'] },
      { q: 'Factor completely: $6x^3 - 24x$', o: ['$6x(x-2)(x+2)$', '$6x(x^2-4)$', '$6(x^3-4x)$', '$2x(3x^2-12)$'], a: 0,
        why: ['GCF = $6x$, then $x^2-4 = (x-2)(x+2)$. FULLY factored: $6x(x-2)(x+2)$.', 'You stopped too early! $x^2 - 4$ is a difference of squares and factors further.', 'You only pulled out 6, not the full GCF of $6x$.', 'You only pulled out $2x$, not the full GCF of $6x$. Always take out the LARGEST common factor.'] },
      { q: 'The parabola $y = -3(x+1)^2 - 5$ opens:', o: ['Down with vertex $(-1,-5)$', 'Up with vertex $(-1,-5)$', 'Down with vertex $(1,-5)$', 'Down with vertex $(-1,5)$'], a: 0,
        why: ['$a = -3 < 0$ so opens down. $h = -1$, $k = -5$, vertex $(-1, -5)$. Correct!', '$a = -3 < 0$ means DOWN, not up.', '$(x+1) = (x-(-1))$ means $h = -1$, not $+1$. Opposite sign!', '$k = -5$, not $+5$. The minus sign means shift down.'] },
      { q: '$x^2 + 4$ can be factored as:', o: ['Cannot be factored over the reals', '$(x+2)(x+2)$', '$(x+2)(x-2)$', '$(x+4)(x+1)$'], a: 0,
        why: ['$a^2 + b^2$ does NOT factor over the reals. There is no "sum of squares" formula. Only $a^2 - b^2$ factors.', '$(x+2)^2 = x^2 + 4x + 4 \\neq x^2 + 4$. Missing the middle term!', '$(x+2)(x-2) = x^2 - 4 \\neq x^2 + 4$. That is difference of squares, not sum.', '$(x+4)(x+1) = x^2 + 5x + 4 \\neq x^2 + 4$.'] },
    ],
    'geometry-measurement': [
      { q: 'In a right triangle with opposite = 5 and hypotenuse = 13, $\\sin \\theta =$', o: ['$5/13$', '$12/13$', '$5/12$', '$13/5$'], a: 0,
        why: ['SOH: $\\sin = \\text{opposite}/\\text{hypotenuse} = 5/13$. Correct!', 'That is $\\cos \\theta$ (adjacent/hypotenuse). Adjacent $= \\sqrt{13^2 - 5^2} = 12$.', 'That is $\\tan \\theta$ (opposite/adjacent).', 'That is $\\csc \\theta$ (hypotenuse/opposite). You flipped the fraction.'] },
      { q: 'Two angles are supplementary. One is $65°$. The other is:', o: ['$115°$', '$25°$', '$295°$', '$90°$'], a: 0,
        why: ['Supplementary means they add to $180°$: $180 - 65 = 115°$.', '$25°$ would be the complement ($90° - 65°$), not the supplement.', '$295°$ would make the total $360°$, not $180°$.', '$90°$ is not supplementary to $65°$: $90 + 65 = 155 \\neq 180$.'] },
      { q: 'The distance between $(1, 2)$ and $(4, 6)$ is:', o: ['$5$', '$7$', '$\\sqrt{7}$', '$25$'], a: 0,
        why: ['$d = \\sqrt{(4-1)^2 + (6-2)^2} = \\sqrt{9 + 16} = \\sqrt{25} = 5$.', '$3 + 4 = 7$ but distance uses Pythagorean theorem, not addition.', '$\\sqrt{3^2 + (-2)^2} = \\sqrt{13}$. Check your subtraction.', '$25 = 3^2 + 4^2$ but you forgot the square root.'] },
      { q: 'The volume of a cylinder with $r = 3$ and $h = 10$ is:', o: ['$90\\pi$', '$30\\pi$', '$60\\pi$', '$900\\pi$'], a: 0,
        why: ['$V = \\pi r^2 h = \\pi(9)(10) = 90\\pi$. Correct!', 'You used $V = \\pi r h = 30\\pi$. Need $r^2$, not $r$.', '$60\\pi = \\pi(6)(10)$. You doubled $r$ instead of squaring it.', 'You squared the height too: $\\pi(9)(100) = 900\\pi$.'] },
      { q: 'In a 30-60-90 triangle, the hypotenuse is 10. The shortest side is:', o: ['$5$', '$5\\sqrt{3}$', '$10\\sqrt{3}$', '$10/\\sqrt{3}$'], a: 0,
        why: ['Ratio $1:\\sqrt{3}:2$. Hypotenuse = $2 \\times$ shortest. So shortest $= 10/2 = 5$.', '$5\\sqrt{3}$ is the MIDDLE side (opposite 60 degrees), not the shortest.', '$10\\sqrt{3}$ is way too big — it exceeds the hypotenuse.', 'Just divide the hypotenuse by 2: $10/2 = 5$. No square roots needed for the shortest side.'] },
      { q: 'Two triangles have angles 40, 60, 80 and 40, 60, 80. They are:', o: ['Similar', 'Congruent', 'Neither', 'Cannot determine'], a: 0,
        why: ['Same angles = same shape = similar (by AA). They may be different sizes.', 'Congruent means same shape AND size. Same angles alone does not guarantee same size.', 'Matching angles is exactly the AA criterion for similarity.', 'Two angles matching is enough — AA similarity is guaranteed.'] },
      { q: 'The midpoint of $(2, 8)$ and $(6, 2)$ is:', o: ['$(4, 5)$', '$(8, 10)$', '$(4, 3)$', '$(2, 3)$'], a: 0,
        why: ['Average: $((2+6)/2, (8+2)/2) = (4, 5)$. Midpoint = average of coordinates.', 'You added without dividing: $(2+6, 8+2) = (8, 10)$.', '$(8-2)/2 = 3$? No, the y-midpoint is $(8+2)/2 = 5$, not $(8-2)/2$.', 'Check: $(2+6)/2 = 4$, not 2.'] },
    ],
    'precalculus': [
